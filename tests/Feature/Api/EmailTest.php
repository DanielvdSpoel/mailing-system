...
it('can filter out archived emails', function () {
    ...
    $this->get(route('emails.index') . '?per_page=100')
        ->assertJson(fn(AssertableJson $json) => $json->has('data')->etc()
            ->has('data', fn(AssertableJson $json) => $json->each(fn($json) => $json->where('is_archived', false)->etc())
            )
        );
});
...
it('can filter out draft emails', function () {
    ...
    $this->get(route('emails.index') . '?per_page=100')
        ->assertJson(fn(AssertableJson $json) => $json->has('data')->etc()
            ->has('data', fn(AssertableJson $json) => $json->each(fn($json) => $json->where('is_draft', false)->etc())
            )
        );
});
...
it('can filter out trashed emails', function () {
    ...
    $this->get(route('emails.index') . '?per_page=100')
        ->assertJson(fn(AssertableJson $json) => $json->has('data')->etc()
            ->has('data', fn(AssertableJson $json) => $json->each(fn($json) => $json->where('is_deleted', false)->etc())
            )
        );

});

it('can filter out spam emails', function () {
    Email::factory()->spam()->count(10)->create();
    Email::factory()->count(10)->create();

    $this->get(route('emails.index') . '?per_page=100')
        ->assertJson(fn(AssertableJson $json) => $json->has('data')->etc()
            ->has('data', fn(AssertableJson $json) => $json->each(fn($json) => $json->where('is_marked_as_spam', false)->etc())
            )
        );
});
